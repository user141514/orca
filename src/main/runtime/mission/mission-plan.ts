...
export type MissionPlanTask = z.infer<typeof MissionPlanTaskSchema>
...
export function buildMissionPlanningPrompt(mission: string): string {
  return [
    ...
    '',
    'First identify the independent evidence-gathering or analysis tracks implied by the objective. Decide based on meaningful parallel benefit, not merely whether one agent could do all the work.',
    'Use {"mode":"single-agent"} for an atomic check, a small focused change, or tightly coupled work with no useful independent track. An explicit request to use only one agent takes precedence over all orchestration guidance below.',
    ...
    'A broad diagnosis or audit spanning independent subsystems should use orchestration even when the request is short and does not mention agents or parallelism. Infer a small set of substantive investigation tracks from the objective; do not split each command into a task.',
    'For example, a whole-computer performance diagnosis has independent compute/memory and storage/system-health evidence to investigate; checking free disk space alone is an atomic single-agent task.',
    'Do not invent extra tasks merely to increase agent count.',
    'Preserve the requested authority in every task: a request to analyze or diagnose does not authorize repairs, installs, settings changes, destructive commands, or intrusive stress tests. Prefer passive, lightweight evidence collection that does not distort the measurements. Read-only investigation and Orca control-plane actions remain allowed.',
    ...
    'A task with subscribesTo must include admission with explicit acceptedTypes; minPriority defaults to normal. Do not declare collaboration fields when tasks are truly independent.',
    "deps only gate execution order; they do not inject another task's result into a worker.",
    "If a task needs another task's output, it must use collaboration topics even when deps also enforce ordering.",
    'Never make a task instruction forbid Orca control-plane actions. worker_done, heartbeat, and ask remain allowed and required protocol actions.',
    '',
...
export function parseMissionPlan(raw: string): MissionPlan {
  ...
  const subscribedTopics = new Set(parsed.data.tasks.flatMap((task) => task.subscribesTo ?? []))
  ...
  for (const task of parsed.data.tasks) {
    ...
    for (const topic of task.requiredPublishesTo ?? []) {
      ...
      if (!subscribedTopics.has(topic)) {
        throw new Error(
          `Mission plan task ${task.key} required publish topic has no subscriber: ${topic}`
        )
      }
    }
