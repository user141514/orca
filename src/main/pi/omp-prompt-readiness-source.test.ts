...
function createHarness(options?: { crypto?: unknown }): Harness {
  const output: string[] = []
  const timers: (() => void)[] = []
  ...
  const source = `${getOmpPromptReadinessSourceLines().join('\n')}\nexport default installOmpPromptReadiness\n`
...
function installReadinessOnlyExtension(args: {
  ownerPid?: string
  paneKey?: string
}): Pick<Harness, 'output' | 'handlers'> & { env: Record<string, string> } {
  const output: string[] = []
...
describe('getOmpPromptReadinessSourceLines', () => {
  ...
  it('emits blocked at installation and ready only after the disabled composer becomes submit-enabled', () => {
  ...
  it('fails closed when an overlay owns focus instead of the composer', () => {
    const composer = createComposer(false)
    const harness = createHarness()
  ...
  it('fails closed for a focused object without the complete OMP composer shape', () => {
    const harness = createHarness()
  ...
  it('returns to blocked when the captured composer is no longer idle', () => {
    const composer = createComposer(false)
    let idle = true
    const harness = createHarness()
  ...
  it('does not accept a different full composer after capturing the startup composer', () => {
  ...
  it('emits blocked when a ready session shuts down', () => {
    const composer = createComposer(false)
    const harness = createHarness()
    const context = createContext({ focused: () => composer })
  ...
  it('lets only the active widget disposal revoke ready after a replacement', () => {
  ...
  it('invalidates an old sentinel immediately on reload before the next session starts', () => {
    const composer = createComposer(false)
    const harness = createHarness()
    const context = createContext({ focused: () => composer })
  ...
  it('does not let a stale session shutdown remove the current widget', () => {
    const composer = createComposer(false)
    const harness = createHarness()
    const context = createContext({ focused: () => composer })
  ...
  it('cancels a queued ready emission when the session shuts down', () => {
    const composer = createComposer(false)
    const harness = createHarness()
    const context = createContext({ focused: () => composer })
  ...
  it('reinstalls a fresh sentinel after reload and accepts an already-enabled full composer', () => {
    const composer = createComposer(false)
    const harness = createHarness()
    const context = createContext({ focused: () => composer })
